...
char *ReadFile(const char *filename, HWSize_t *size) {
  ...
  while (left_to_read > 0) {
    ...
    if (result == 0) {
      left_to_read = 0;
    } else if (result == -1) {  // handle the special case
      ...
      // update num of bytes read, and num of bytes left to read
      numread += result;
      left_to_read -= result;
    }
  }
...
static void LoopAndInsert(HashTable tab, char *content) {
  ...
  // STEP 7.
  // This is the interestingif part of Part A!
  //
  ...
  while (1) {
    if (*curptr == '\0') {
      ...
      *curptr = tolower(*curptr);  // it is part of a word
    } else {
      *curptr = '\0';
      if (curptr != content && isalpha(*(curptr - 1)) != 0)
        // record the word in the hashtable
...
static void AddToHashtable(HashTable tab, char *word, DocPositionOffset_t pos) {
  ...
  if (retval == 1) {
    ...
    // allocate space for wp->word
    str = (char *) malloc(strlen(word) + 1);
    Verify333(str != NULL);

    // copy the content of word to wp->word
    strncpy(str, word, strlen(word) + 1);
    wp->word = str;

    // allocate space for wp->position
    wp->positions = AllocateLinkedList();
    Verify333(wp->positions != NULL);

    // append the linked list with new position of word
    retval = AppendLinkedList(wp->positions, (LLPayload_t) ((intptr_t) pos));
    Verify333(retval != 0);

    // insert the key and value into hashtable
    kv.key = hashKey;
    kv.value = (LLPayload_t) wp;
