...
int MIAddPostingList(MemIndex index, char *word, DocID_t docid,
  ...
  if (res == 0) {
    ...
    Verify333(wds != NULL);
    // insert word into worddocset
    wds->word = word;

    // allocate a new hashtable
    // insert the hashtable into worddocset
    ...
    // insert worddocset into inverted index
...
LinkedList MIProcessQuery(MemIndex index, char *query[], uint8_t qlen) {
  ...
  // get an iterator on docIDs table
  HTIter it = HashTableMakeIterator(wds->docIDs);
  Verify333(it != NULL);

  while (!HTIteratorPastEnd(it)) {
    res = HTIteratorGet(it, &kv);  // get id and positions of the word
    Verify333(res == 1);
    SearchResult *sres = (SearchResult *) malloc(sizeof(SearchResult));
    Verify333(sres != NULL);
    sres->docid = (DocID_t) kv.key;
    sres->rank = NumElementsInLinkedList((LinkedList)kv.value);

    // append the SearchResult onto retlist
    bool success = AppendLinkedList(retlist, sres);
    Verify333(success);
    HTIteratorNext(it);
  }
  HTIteratorFree(it);
  ...
  for (i = 1; i < qlen; i++) {
    ...
    SearchResult *sres;
    for (j = 0; j < ne; j++) {
      LLIteratorGetPayload(llit, (LLPayload_t *)&sres);
      res = LookupHashTable(wds->docIDs, sres->docid, &kv);
      Verify333(res != -1);

      // delete docID from the search result list
      if (res == 0) {
        LLIteratorDelete(llit, &free);
      } else {  // update the search result list's rank
        sres->rank += NumElementsInLinkedList((LinkedList)kv.value);
        LLIteratorNext(llit);
      }
    }
