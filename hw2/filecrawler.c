  ...
  while (1) {
    ...
    if (res == 0) {
      ...
      if (S_ISREG(nextstat.st_mode)) {
        HandleFile(newfile, doctable, index);  // it's a regular file
      } else if (S_ISDIR(nextstat.st_mode)) {
        DIR *newdir = opendir(newfile);  // it's a directory

        // if fails to open directory, free the malloced filepath, and check
        // others
        if (newdir == NULL) {
          free(newfile);
          continue;
