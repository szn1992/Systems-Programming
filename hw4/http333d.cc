...
// helper method that checks if a file is readable
bool readable(std::string fname) {
  char *filename = new char[fname.length()+1];
  FILE *file;

  strncpy(filename, fname.c_str(), fname.length());
...
void GetPortAndPath(int argc,
  ...
  for (int i = 3; i < argc; i++) {
    ...
    if (readable(str))
      (*indices).push_back(str);  // the index is readable
    else
      Usage(argv[0]);
  }
}
