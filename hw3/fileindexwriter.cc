...
HWSize_t WriteIndex(MemIndex mi, DocTable dt, const char *filename) {
  ...
  // MISSING:
  int heres = WriteHeader(f, dtres, mires);
  if (heres == 0) {
    fclose(f);
    unlink(filename);
    return 0;
...
static HWSize_t WriteDocidDocnameFn(FILE *f,
  ...
  // docID + filename length + filename
  return 8 + 2 + slen_ho;
}
...
static HWSize_t WriteDocPositionListFn(FILE *f,
  ...
  for (i = 0; i < num_pos_ho; i++) {
    ...
    if (fseek(f, sizeof(docid_element_header) + offset, SEEK_SET) != 0)
      return 0;
  ...
  return sizeof(docid_element_header) + num_pos_ho *
                               sizeof(docid_element_position);
}
...
static HWSize_t WriteBucket(FILE *f,
  ...
  if (chainlen_ho > 0) {
    ...
    for (j = 0; j < chainlen_ho; j++) {
      ...
      // MISSING:
      element_position_rec epr = {(IndexFileOffset_t)nextelpos};
      epr.toDiskFormat();
