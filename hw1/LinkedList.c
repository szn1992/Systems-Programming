...
bool PopLinkedList(LinkedList list, LLPayload_t *payload_ptr) {
  ...
  LinkedListNodePtr oldHead = list->head;  // the node to be popped
  *payload_ptr = oldHead->payload;  // stores the payload stored
                                    // in the node to be popped
...
bool SliceLinkedList(LinkedList list, LLPayload_t *payload_ptr) {
  ...
  LinkedListNodePtr oldTail = list->tail;  // the node to be popped
  *payload_ptr = oldTail->payload;  // stores the payload stored
                                    // in the node to be popped
...
bool LLIteratorDelete(LLIter iter,
  ...
  if (iter->list->num_elements == 1) {
    ...
    // iter points in the middle
    iter->node = temp->next;
    temp->prev->next = iter->node;
    iter->node->prev = temp->prev;
  }
